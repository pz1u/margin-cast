...
- Phase 5 기상청 API: 카카오 주소 검색·매장 위경도 변환·1~5일 실제 예보 Simulation 연결 완료, 7일 중기예보 권한·결합 대기
...
- Phase 11 피드백·Web·API: 실험 계획·실제 결과 기록과 로컬 웹사이트 완료, Agent 연결·공개 배포 대기
...
| 11 | 피드백·Web·API | 실제 실험 오차 평가, 사용자 흐름, 배포, 모델 보정 경로 검증 |

## 후속 확장 항목

- Variable Cost 모델 확장: 현재 엔진의 기여이익은 식재료 원가와 플랫폼·결제 수수료만 뺀다.
  포장비, 배달 대행료, 요청별 수수료율을 반영하려면 `contribution_profit` 정의와 데이터 생성·검증,
  엔진 입력 계약을 함께 확장해야 하며 UI만 먼저 추가하지 않는다.
- 매장 위치: 현재 위치 사용과 매장 검색을 기존 카카오·기상청 위치 계약에 연결한다(H1).
- 실험 피드백 웹 화면 복원.
...
본체 구현 범위와 완료 조건은 [MarginCast Agent 본체 기획서](agent-body-plan.md)를 기준으로 한다.
